...
docker compose up -d
```
باید پورت 9091 باز باشه روی سرور،‌برای تست کردنش آدرس زیر رو تو مرورگر باز کنید. بجای `<node_endpoint>` باید IP نود خودتون رو بزارید

```
http://<node_endpoint>:9091/eigen/node
```
اگه جواب نگرفتید یعنی پورت بسته اس. اگه از فایروال ufw استفاده میکنید،‌ با دستور زیر پورت رو باز کنید و دوباره تست کنید
```
sudo ufw allow 9091
```
